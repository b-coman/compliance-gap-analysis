...
3. **GDPR consolidated file (`uk-gdpr-articles-relevant.txt`) was loaded but produced zero chunks** (per plan). The file was loaded by ING-01 for corpus completeness; ING-02's dispatcher returned `[]` because the per-article files cover the same content with finer granularity. *Subsequently removed entirely in the post-Pass-2 corpus audit (2026-05-08): a file loaded but never chunked is dead weight in the manifest, so we deleted it from disk and removed its references from `_STATIC_LABELS` and `_dispatch_chunker`.*
