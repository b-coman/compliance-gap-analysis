...
- That 3B is the production default. The local 1.5B path remains the demo-reliability default (no Colab dependency, no API keys, no daily limits). 3B and 7B are evaluation comparison points; deciding to ship 3B locally would require packaging a different model on the demo machine.
- That every failure mode is now categorised. Q2 produced different reading errors at 3B (internal inconsistency) and 7B (misread §3.4 timing). Both are different from 1.5B's FRIA leak. The full failure-mode taxonomy below 1.5B-leak-territory is not yet exhaustively mapped.
- That the FRIA-leak threshold generalises across model families. Stage 8 tested Qwen 2.5 only. A Gemma 2-2B comparison (similar parameter count to Qwen 3B but different family) would test whether the suppression is parameter-count-driven or training-corpus-driven; not yet run.
- That CPU offloading at 7B is acceptable. It is a workaround for this specific session, not a recommended pattern. Documented for the next reader so they don't conclude *"7B is unusable"*.

**For the report's Critical Analysis dimension,** Stage 8 contributes:
- The cleanest empirical narrative arc the project has on prompt-design-vs-model-scale-vs-architecture: V3 → V4 (prompt-design negative finding) → 3B (model-scale positive finding for FRIA leak) → 7B (model-scale positive finding for sub-clause analysis). Three independent mitigation hypotheses, three different empirical outcomes.
- An evidence-based claim that *different failure modes have different scale thresholds* — supports a structured-error-categories framing rather than a one-size-fits-all "improve the LLM" recommendation.
- A defensible cost-vs-quality position on which model to run when. 1.5B for daily iteration; 3B for the demo-quality Q5 finding (cheap to reproduce); 7B for the rare query that needs multi-step reasoning (expensive to reproduce, slower on commodity hardware). Each tier earns its place in the deployment story.
